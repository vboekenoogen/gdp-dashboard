...
# Add this new function to detect file encoding
    ...
    try:
        ...
        if encoding and confidence > 0.7:
            try:
                ...
                st.warning(f"Failed to read with detected encoding {encoding}, trying alternatives...")
        
        # If detection failed or confidence is low, try common encodings
        ...
        st.warning("⚠️ Read file with UTF-8 encoding, replacing invalid characters with '�'")
        return df
...
def main():
    ...
    if uploaded_file is not None:
        try:
            # Use the new encoding-aware function to read the file
            df_raw = read_csv_with_encoding(uploaded_file)
